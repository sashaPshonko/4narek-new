...
## Вектор: FunTime min/max не принимать втупую

Бот **лот выставляет** по цифре FunTime (`needPrice` + retry). Каталог Go (закуп/витрина флота) — отдельный слой; раньше `set_min` / `set_max` писались почти как есть.

**Факт (02.09.2026, `шлем-1.21`):** `server_min` 1.60M → 3.55M при живых buy/sell; `hold_manual_min` запретил ↓, held 42→94. Не рынок, а скачок листинга. В БД `server_max` только вниз (медиана ~−7%). Штаны 600→950 (+350к) при закупках — скорее сдвиг пола.

**Живой фильтр (черновик):** каталог не поднимаем, если FunTime хочет **+≥1кк** и за 3 цикла уже были **закупки**. Мелкий мин проходит. Нет закупок — большой мин можно принять. Max вниз — всегда. Код: `serverFunTimeRaiseAnomalous` в `pricing.go`.

**Дальше — не копировать границу 1:1**, а подстраивать sell под min/max и **мерить прибыль**:

1. `server_price_events` + циклы до/после (`sales`, `buys`, `try_sells`, `held`, цена)
2. `fwd_profit_1..3` после min/max vs HOLD / vs те же часы без скачка
3. Отличить новый пол АХ (штаны) от разового глюка (шлем 3.55M)
4. Лот не стопать; каталог двигать, только если fwd не хуже
...
| Слепо писать FunTime min/max в каталог | Частично снято (+≥1кк и закупки); дальше — адаптация по fwd, вектор выше |
